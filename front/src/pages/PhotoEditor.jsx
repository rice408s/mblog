...
export function PhotoEditor() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 pt-24 pb-12">
      ...
      {photos.length > 0 && (
        <div className="mb-8">
          <h2 className="text-xl font-light text-white/80 mb-4">已添加的图片</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {photos.map((url, index) => (
              <div 
                key={index}
                className="group relative aspect-square rounded-lg overflow-hidden bg-white/[0.02] border border-white/10"
              >
                <img
                  src={url}
                  alt={`图片 ${index + 1}`}
